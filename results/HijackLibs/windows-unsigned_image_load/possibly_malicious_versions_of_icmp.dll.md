...
event.type="ModuleLoad" and (endpoint.os="windows" and (module.path contains "\icmp.dll" and (not (not (module.path matches "\.*") or not (module.path matches "\.*") or not (module.path matches "\.*")))))
```
