...
event.type="Module Load" and (endpoint.os="windows" and (module.path contains "\\fwbase.dll" and (not (not (module.path matches "\.*") or not (module.path matches "\.*") or not (module.path matches "\.*")))))
```
