...
event.type="Process Creation" and (endpoint.os="windows" and (src.process.image.path contains "\\aeroadmin.exe" or tgt.process.image.path contains "\\aeroadmin.exe"))
```
...
detection:
    selection_parent:
        ParentImage|endswith: '\\aeroadmin.exe'
    selection_image:
        Image|endswith: '\\aeroadmin.exe'
