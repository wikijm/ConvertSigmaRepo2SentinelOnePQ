...
event.type="Process Creation" and (endpoint.os="windows" and (src.process.image.path contains "\\Duplicati.Server.exe" or tgt.process.image.path contains "\\Duplicati.Server.exe"))
```
...
detection:
    selection_parent:
        ParentImage|endswith: '\\Duplicati.Server.exe'
    selection_image:
        Image|endswith: '\\Duplicati.Server.exe'
