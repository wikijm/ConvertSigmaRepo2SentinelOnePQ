...
event.type="Process Creation" and (endpoint.os="windows" and (src.process.image.path contains "\\mRemoteNG.exe" or tgt.process.image.path contains "\\mRemoteNG.exe"))
```
...
detection:
    selection_parent:
        ParentImage|endswith: '\\mRemoteNG.exe'
    selection_image:
